...
export function getBusinessImage(business) {
  // Use first available image directly (no over-validation)
  if (business?.images?.length > 0) {
    return business.images[0]
  }
  
  // If business has a single image property
  if (business?.image) {
...
export function getDestinationImage(destination) {
  ...
  // If destination has real images, use the first valid one
  if (destination?.images?.length > 0) {
    const validImage = getFirstValidImage(destination.images)
    if (validImage) {
      return validImage
    }
  }
  ...
  if (destination?.image) {
    const normalizedImage = normalizeImageUrl(destination.image)
    if (normalizedImage) {
...
export function getActivityImage(activity) {
  // If activity has real images, use the first one
  if (activity?.images?.length > 0) {
    const validImage = getFirstValidImage(activity.images)
    if (validImage) {
      return validImage
    }
  }
  
  // If activity has a single image property
  if (activity?.image) {
    const normalizedImage = normalizeImageUrl(activity.image)
    if (normalizedImage) {
      return normalizedImage
    }
  }
